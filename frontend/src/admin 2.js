...
function populateMapItemsTable(items) {
  ...
  tr.innerHTML = `
      <td>${idx + 1}</td>
      <td><span class="mapitem-name" data-id="${item._id}">${item.name || ''}</span> <button class="edit-mapitem-name-btn" data-id="${item._id}">修改</button></td>
      <td><select class="mapitem-category-select" data-id="${item._id}">${categoryOptions}</select></td>
      <td><span class="mapitem-address" data-id="${item._id}">${item.formattedAddress || ''}</span> <button class="edit-mapitem-address-btn" data-id="${item._id}">修改</button></td>
      <td><span class="mapitem-description" data-id="${item._id}">${item.description || ''}</span> <button class="edit-mapitem-description-btn" data-id="${item._id}">修改</button></td>
      <td><span class="mapitem-phone" data-id="${item._id}">${item.phone || ''}</span> <button class="edit-mapitem-phone-btn" data-id="${item._id}">修改</button></td>
      <td><button class="edit-mapitem-openinghours-btn" data-id="${item._id}">編輯</button></td>
      <td><span class="mapitem-longitude" data-id="${item._id}">${item.longitude || ''}</span> <button class="edit-mapitem-longitude-btn" data-id="${item._id}">修改</button></td>
      <td><span class="mapitem-latitude" data-id="${item._id}">${item.latitude || ''}</span> <button class="edit-mapitem-latitude-btn" data-id="${item._id}">修改</button></td>
      <td><span class="mapitem-website" data-id="${item._id}">${item.website || ''}</span> <button class="edit-mapitem-website-btn" data-id="${item._id}">修改</button></td>
      <td><span class="mapitem-photo" data-id="${item._id}">${photoUrl ? `<a href='${photoUrl}' target='_blank'>檢視</a>` : '無'}</span> <button class="edit-mapitem-photo-btn" data-id="${item._id}">修改</button></td>
      <td><button class="delete-mapitem-btn" data-id="${item._id}">刪除</button></td>
    `;
    tableBody.appendChild(tr);
  });
  // 編輯按鈕
  document.querySelectorAll('.edit-mapitem-name-btn').forEach(btn => {
    btn.addEventListener('click', (e) => {
      const id = btn.dataset.id;
      const span = document.querySelector(`.mapitem-name[data-id='${id}']`);
      const oldValue = span ? span.textContent : '';
      const newValue = prompt('請輸入新名稱', oldValue);
      if (newValue !== null && newValue !== oldValue) updateMapItemField(id, 'name', newValue);
    });
  });
  ...
  document.querySelectorAll('.edit-mapitem-address-btn').forEach(btn => {
    btn.addEventListener('click', (e) => {
      const id = btn.dataset.id;
      const span = document.querySelector(`.mapitem-address[data-id='${id}']`);
      const oldValue = span ? span.textContent : '';
      const newValue = prompt('請輸入新地址', oldValue);
      if (newValue !== null && newValue !== oldValue) updateMapItemField(id, 'formattedAddress', newValue);
    });
  });
  document.querySelectorAll('.edit-mapitem-description-btn').forEach(btn => {
    btn.addEventListener('click', (e) => {
      const id = btn.dataset.id;
      const span = document.querySelector(`.mapitem-description[data-id='${id}']`);
      const oldValue = span ? span.textContent : '';
      const newValue = prompt('請輸入新描述', oldValue);
      if (newValue !== null && newValue !== oldValue) updateMapItemField(id, 'description', newValue);
    });
  });
  document.querySelectorAll('.edit-mapitem-phone-btn').forEach(btn => {
    btn.addEventListener('click', (e) => {
      const id = btn.dataset.id;
      const span = document.querySelector(`.mapitem-phone[data-id='${id}']`);
      const oldValue = span ? span.textContent : '';
      const newValue = prompt('請輸入新電話', oldValue);
      if (newValue !== null && newValue !== oldValue) updateMapItemField(id, 'phone', newValue);
    });
  });
  document.querySelectorAll('.edit-mapitem-openinghours-btn').forEach(btn => {
    btn.addEventListener('click', (e) => {
      const id = btn.dataset.id;
      const item = items.find(i => i._id === id);
      showOpeningHoursModal(id, item.openingHours || []);
    });
  });
  document.querySelectorAll('.edit-mapitem-longitude-btn').forEach(btn => {
    btn.addEventListener('click', (e) => {
      const id = btn.dataset.id;
      const span = document.querySelector(`.mapitem-longitude[data-id='${id}']`);
      const oldValue = span ? span.textContent : '';
      const newValue = prompt('請輸入新經度', oldValue);
      if (newValue !== null && newValue !== oldValue) updateMapItemField(id, 'longitude', Number(newValue));
    });
  });
  document.querySelectorAll('.edit-mapitem-latitude-btn').forEach(btn => {
    btn.addEventListener('click', (e) => {
      const id = btn.dataset.id;
      const span = document.querySelector(`.mapitem-latitude[data-id='${id}']`);
      const oldValue = span ? span.textContent : '';
      const newValue = prompt('請輸入新緯度', oldValue);
      if (newValue !== null && newValue !== oldValue) updateMapItemField(id, 'latitude', Number(newValue));
    });
  });
  document.querySelectorAll('.edit-mapitem-website-btn').forEach(btn => {
    btn.addEventListener('click', (e) => {
      const id = btn.dataset.id;
      const span = document.querySelector(`.mapitem-website[data-id='${id}']`);
      const oldValue = span ? span.textContent : '';
      const newValue = prompt('請輸入新網址', oldValue);
      if (newValue !== null && newValue !== oldValue) updateMapItemField(id, 'website', newValue);
    });
  });
  document.querySelectorAll('.edit-mapitem-photo-btn').forEach(btn => {
    btn.addEventListener('click', (e) => {
      const id = btn.dataset.id;
      const item = items.find(i => i._id === id);
      const oldValue = (item.photos && item.photos.length > 0 && item.photos[0].url) ? item.photos[0].url : '';
      const newValue = prompt('請輸入新圖片連結', oldValue);
      if (newValue !== null && newValue !== oldValue) updateMapItemField(id, 'photos', newValue ? [{ url: newValue }] : []);
    });
  });
...
addRestaurantBtn.addEventListener('click', async function() {
  ...
  const formattedAddress = document.querySelector('.restaurantAddress').value;
  const description = document.querySelector('.restaurantDescription').value;
  const phone = document.querySelector('.restaurantPhone').value;
  // 經緯度合併 input
  const lngLatStr = document.querySelector('.restaurantLngLat').value;
  let longitude = '', latitude = '';
  if (lngLatStr) {
    const parts = lngLatStr.split(',').map(s => s.trim());
    longitude = Number(parts[0]);
    latitude = Number(parts[1]);
  }
  const website = document.querySelector('.restaurantUrl').value;
  const photoUrlInput = document.querySelector('.restaurantPhotoUrl');
  const photoUrl = photoUrlInput ? photoUrlInput.value : '';
  const openingHours = collectOpeningHours();

  if (!name || !formattedAddress || !description || !longitude || !latitude) {
    alert('請填寫所有必填欄位');
    return;
  }
  ...
  try {
    ...
    document.querySelector('.restaurantAddress').value = '';
    document.querySelector('.restaurantDescription').value = '';
    document.querySelector('.restaurantPhone').value = '';
    document.querySelector('.restaurantLngLat').value = '';
    document.querySelector('.restaurantUrl').value = '';
    if (photoUrlInput) photoUrlInput.value = '';
    openingHoursList.innerHTML = `<tr><td colspan="4" style="text-align: center;">無資料</td></tr>`;
    fetchMapItems();
  } catch (err) {
    alert('新增失敗');
  }
